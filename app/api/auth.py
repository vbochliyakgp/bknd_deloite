...
async def login_employee(
    ...
    Login for employees
    """
    employee = db.query(Employee).filter(Employee.employee_id == employee_login.employee_id).first()
    
    if not employee or not verify_password(employee_login.password, employee.hashed_password):
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
...
async def login_for_access_token(
    ...
    employee = db.query(Employee).filter(Employee.employee_id == form_data.username).first()
