...
async def update_user(
    ...
    if user_update.username is not None:
        ...
        if user_update.username != user.username and db.query(User).filter(User.username == user_update.username).first():
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
    ...
    if user_update.email is not None:
        ...
        if user_update.email != user.email and db.query(User).filter(User.email == user_update.email).first():
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
...
async def create_employee(
    ...
    existing_employee = (
        db.query(Employee)
        .filter((Employee.employee_id == employee.employee_id) | (Employee.email == employee.email))
        .first()
    )
...
async def update_employee(
    ...
    if employee_update.email is not None:
        # Check if email is being changed and if it's already in use
        if employee_update.email != employee.email and db.query(Employee).filter(Employee.email == employee_update.email).first():
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Email already in use"
            )
    ...
    if employee_update.manager_id is not None:
        ...
        employee.manager_id = employee_update.manager_id if employee_update.manager_id > 0 else None
