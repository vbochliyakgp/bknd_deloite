...
class AnalyticsService:
    ...
    def get_employee_data(db: Session, employee_id: int) -> Dict[str, Any]:
        ...
        leave_taken = sum((l.end_date - l.start_date).days + 1 for l in leave_data 
        ...
        avg_hours = sum(a.hours_worked for a in activity_data) / (len(activity_data) or 1)
        avg_after_hours = sum(a.after_hours_work for a in activity_data) / (len(activity_data) or 1)
        ...
        return {
            ...
            "performance_data": {
                ...
                "review_period": latest_performance.review_period if latest_performance else None,
                "promotion_eligible": latest_performance.promotion_eligible if latest_performance else None
            },
            "rewards_data": {
                "count": len(rewards),
                "recent_rewards": [
    ...
    @staticmethod
    def get_employee_analytics(db: Session, employee_id: int) -> EmployeeSessionAnalytics:
        """
        ...
        negative_emotions_count = sum(1 for e in recent_vibes if e in [EmotionZone.FRUSTRATED, EmotionZone.SAD])
        ...
        if employee_data["performance_data"]["rating"] and employee_data["performance_data"]["rating"] < 3:
            risk_factors.append("Below average performance rating")
        
        # Generate recommendations
        recommendations = []
        if "High frequency of negative emotions" in risk_factors:
            recommendations.append("Schedule a one-on-one wellness check with manager or HR")
        
        if "Consistently long working hours" in risk_factors or "Significant after-hours work" in risk_factors:
            recommendations.append("Evaluate workload distribution and consider reallocation")
        
        if "Unused leave accumulation" in risk_factors:
            recommendations.append("Encourage taking planned time off for work-life balance")
    ...
    def identify_at_risk_employees(db: Session) -> List[EmployeeAlert]:
        ...
        for employee in employees:
            ...
            recent_vibes = (
                ...
                .filter(VibemeterResponse.response_date >= date.today() - timedelta(days=10))
                .order_by(VibemeterResponse.response_date)
                .all()
            )
            ...
            if len(negative_emotions) >= 3 and len(recent_vibes) >= 5:
                alerts.append(EmployeeAlert(
                    ...
                    alert_severity="High" if len(negative_emotions) >= 4 else "Medium",
            ...
            leave_records = db.query(Leave).filter(Leave.employee_id == employee.id).all()
            leave_taken = sum((l.end_date - l.start_date).days + 1 for l in leave_records 
                            if l.start_date.year == date.today().year)
    ...
    def generate_daily_report(db: Session, report_date: Optional[date] = None) -> DailyReport:
        ...
        for dept in departments:
            ...
            department_breakdown[dept_name] = {
                ...
                "response_rate": len(dept_responses) / dept_employees if dept_employees else 0,
        ...
        negative_emotions = emotion_distribution["Frustrated"] + emotion_distribution["Sad"]
        ...
        if response_rate < 0.7:
            recommendations.append("Improve Vibemeter participation through reminders and engagement")
        ...
        return DailyReport(
            ...
            engagement_metrics={
                ...
                ]) / response_count if response_count else 0, 2)
            },
